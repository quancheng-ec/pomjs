...
module.exports = function(compiler, options) {
	...
	function handleRangeHeaders(content, req, response) {
		if (req.headers['Accept-Ranges']) response.set('Accept-Ranges', 'bytes');
		if (req.headers.range) {
			var ranges = parseRange(content.length, req.headers.range);

			// unsatisfiable
			if (-1 == ranges) {
				response.set('Content-Range', 'bytes */' + content.length);
				response.status = 416;
				return content;
			}

			// valid (syntactically invalid/multiple ranges are treated as a regular responseponse)
	...
	function webpackDevMiddleware(ctx, next) {
		...
		function processRequest() {
			...
			content = handleRangeHeaders(content, req, response);
			response.set("Access-Control-Allow-Origin", "*"); // To support XHR, etc.
			response.set("Content-Type", mime.lookup(filename));
			response.set("Content-Length", content.length);
			if(options.headers) {
				for(var name in options.headers) {
					response.set(name, options.headers[name]);
				}
			}
			ctx.body = content;
		}
