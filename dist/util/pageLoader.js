...
var find = function find(f) {
  ...
  if (!_isProduction && apis[api] && !process.env.DEBUG) {
    var apiModule = require.cache[api];
    if (apiModule.parent) {
      apiModule.parent.children.splice(apiModule.parent.children.indexOf(apiModule), 1);
    }
  ...
  if (!fs.exists(vue)) {
    var _serverEntry = process.env.serverEntry || Path.join(__dirname, '../../vue.js');
    fs.copy(_serverEntry, vue);
  }
...
module.exports = {
  ...
  compileRun: function () {
    var _ref = (0, _bluebird.coroutine)(function* (cb) {
